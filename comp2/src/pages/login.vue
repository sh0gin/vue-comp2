...
const login = async () => {
  ...
  if (response.status == 200) {
    localStorage.setItem('activeToken', result.data.token);
    activeToken.value = result.data.token;
    router.push('/courses');
  } else if (response.status == 422) {
    Object.keys(result.data.errors).forEach(element => {
      errors[`${element}_error`] = result.data.errors[element] ? result.data.errors[element][0] : null;
    })
...
<template>
  ...
  <main>
    <form @submit.prevent="login()">
      ...
      <label>Пароль: <input v-model="password" type='password' name="password"></label>
